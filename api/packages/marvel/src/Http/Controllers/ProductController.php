...
class ProductController extends CoreController
{
    ...
    public function fetchProducts(Request $request)
    {
        ...
        // IndoBangla: single-box product search that matches the English name,
        // Bangla name, slug, OR author name (kept as its own `text` param so
        // existing storefront calls, which never send it, are unaffected).
        if ($text = trim((string) $request->input('text', ''))) {
            $products_query = $products_query->where(function ($w) use ($text) {
                $like = "%{$text}%";

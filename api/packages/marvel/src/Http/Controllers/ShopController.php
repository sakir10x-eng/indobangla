...
class ShopController extends CoreController
{
    ...
    public function fetchShops(Request $request)
    {
        ...
        // #3 — Hide deactivated/archived vendors from the public storefront. Super
        // admins still see every shop (so they can review/reactivate); customers and
        // unauthenticated visitors only see active shops.
        $user = $request->user();
        $isPrivileged = $user && $user->hasPermissionTo(Permission::SUPER_ADMIN);
        if (!$isPrivileged) {

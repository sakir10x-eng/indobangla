...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    protected static function boot()
    {
        ...
        // Every user carries an 8-digit membership card number from the moment they sign up.
        static::creating(function ($user) {
            if (empty($user->membership_no)) {
                do {
                    $no = (string) random_int(10000000, 99999999);
                } while (static::withoutGlobalScopes()->where('membership_no', $no)->exists());
                $user->membership_no = $no;
            }
        });
    }

...
import { CHALLENGE_COUPON_KEY } from '@/framework/challenge';
...
const Coupon = ({ theme, subtotal }: { theme?: 'dark'; subtotal: number }) => {
  ...
  // A discount won in the 1-minute challenge applies itself — the customer earned it in
  // under a minute, so making them copy a code across would be a poor way to end the game.
  useEffect(() => {
    if (coupon || !subtotal) return;
    const won = localStorage.getItem(CHALLENGE_COUPON_KEY);
    if (!won) return;
    localStorage.removeItem(CHALLENGE_COUPON_KEY);
    verifyCoupon({ code: won, sub_total: subtotal, item: items });

...
const Footer = ({ variant = 'light' }: { variant?: 'light' | 'dark' }) => {
  ...
  return (
    <div
      className={cn(
        'flex w-full flex-col border-t border-gray-800 border-t-border-100 bg-white px-5 md:px-10 lg:border-b-8 lg:px-[50px] xl:px-16',
        variant === 'dark' && 'bg-neutral-900 border-0 lg:border-0 dark',
      )}
    >
      {/* Top */}
      ...
      <div
        className={cn(
          'flex flex-col items-center w-full gap-2 pt-8 pb-20 mt-8 border-t lg:mt-0 lg:flex-row lg:justify-between lg:border-t-0 lg:pb-12',
          variant === 'light' ? 'border-t-gray-200' : 'border-t-gray-500',
        )}
      >

...
class Client {
  products = {
    ...
    latest: (params: Partial<LatestProductQueryOptions>) =>
      HttpClient.get<Product[]>(API_ENDPOINTS.LATEST_PRODUCTS, params),
  ...
  wishlist = {
    ...
    // Idempotent — the API firstOrCreate's, so saving an already-saved book is a no-op
    // rather than a toggle that would quietly un-save it.
    add: (input: { product_id: string }) =>
      HttpClient.post<Wishlist>(API_ENDPOINTS.WISHLIST, input),
    remove: (id: string) =>
  ...
  // 1-minute book challenge. The clock, the book count and the discount are all decided
  // server-side — these calls only report what the customer did.
  challenge = {
    status: () => HttpClient.get<any>('challenge/status'),
    start: () => HttpClient.post<any>('challenge/start', {}),
    add: (input: { product_id: string | number; token: string }) =>
      HttpClient.post<any>('challenge/add', input),
  ...
  customPage = {
    get: () => HttpClient.get<CustomPagesData>(API_ENDPOINTS.CUSTOM_PAGE),
  };
}

...
echo "${username}"
echo "${ip_address}"
ssh -o StrictHostKeyChecking=no -l "${username}" "${ip_address}" "sudo mkdir -p /var/www/indobangla-laravel; sudo chown -R \$USER:\$USER /var/www; sudo apt install zip unzip";
...
if [ -f "./api.zip" ] && [ -f "./deployment.zip" ]; then
    ...
    scp "./api.zip" "${username}@${ip_address}:/var/www/indobangla-laravel"
    echo "uploaded api.zip to server"
    ssh -o StrictHostKeyChecking=no -l "${username}" "${ip_address}" "unzip /var/www/indobangla-laravel/api.zip -d /var/www/indobangla-laravel";

    # echo "Enter your deployment.zip file path"
    # read deployment_source_path
    echo 'Uploading deployment.zip to server...'
    scp "./deployment.zip" "${username}@${ip_address}:/var/www/indobangla-laravel"
    echo 'uploaded deployment.zip to server'
    ssh -o StrictHostKeyChecking=no -l "${username}" "${ip_address}" "unzip /var/www/indobangla-laravel/deployment.zip -d /var/www/indobangla-laravel";
else

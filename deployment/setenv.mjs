...
await $`sudo rm -f /etc/nginx/sites-enabled/indobangla`
await $`sudo rm -f /etc/nginx/sites-available/indobangla`
await $`sudo touch /etc/nginx/sites-available/indobangla`
await $`sudo chmod -R 777 /etc/nginx/sites-available/indobangla`
...
if(whichConfig == 1) {
    ...
    await $`sudo echo 'server {
        ...
        # For API
        location /backend {
            alias /var/www/indobangla-laravel/api/public;
    ...
    }' > '/etc/nginx/sites-available/indobangla'`;
...
echo(chalk.blue('\nEnabling the config'))
await $`sudo ln -s /etc/nginx/sites-available/indobangla /etc/nginx/sites-enabled/`

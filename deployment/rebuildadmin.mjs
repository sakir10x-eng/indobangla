...
if (whichConfig == 1) {
    ...
    await $`ssh -o StrictHostKeyChecking=no -l ${username} ${ip_address} "rm -rf /var/www/indobangla-laravel/admin.zip /var/www/indobangla-laravel/admin";`
    // let front_end_source_path = await question('Enter frontend.zip source path (ex: /home/../indobangla-laravel/frontend.zip): ')
    let front_end_source_path = "./admin.zip";
    echo("Uploading admin.zip to server, Please wait...")
    await $`scp ${front_end_source_path} ${username}@${ip_address}:/var/www/indobangla-laravel`

    echo(chalk.green("Uploaded admin.zip to server"))
    await $`ssh -o StrictHostKeyChecking=no -l ${username} ${ip_address} "unzip /var/www/indobangla-laravel/admin.zip -d /var/www/indobangla-laravel";`

    echo('Install Node For admin')
    await $`ssh -o StrictHostKeyChecking=no -l ${username} ${ip_address} "yarn --cwd /var/www/indobangla-laravel/admin/rest";`
    ...
    echo('Install Node For admin')
    await $`ssh -o StrictHostKeyChecking=no -l ${username} ${ip_address} "yarn --cwd /var/www/indobangla-laravel/admin/graphql";`

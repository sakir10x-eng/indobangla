# Automation scripts for Indobangla
...
To upload the zipped `api` and `deployment` files to server you need to run the below command form your indobangla project root
...
> files path and the path will be look like `/home/your_project_folder_path/indobangla-laravel/api.zip` for api.zip file so forth for `deployment.zip`
...
```bash
    bash /var/www/indobangla-laravel/deployment/nodesetup.sh
````

#### Nginx Setup And Settings - RUN on Virtual Server

```bash
    zx /var/www/indobangla-laravel/deployment/setenv.mjs
````

#### Backend build - RUN on Virtual Server

```bash
    sudo zx /var/www/indobangla-laravel/deployment/backendbuildscript.mjs
```

#### Frontend build script - RUN on Local PC
Run the below command from your indobangla-laravel project root
...
```bash
    zx /var/www/indobangla-laravel/deployment/frontendrunscript.mjs
```

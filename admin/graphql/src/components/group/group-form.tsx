...
import { FormValues } from '@/components/group/form-parts/type';
import { typeIcon } from '@/components/group/group-icons';
import { groupValidationSchema } from '@/components/group/group-validation-schema';
...
import { FlashSale, Product, Type } from '__generated__/__types__';
...
export default function CreateOrUpdateGroupForm({
  initialValues,
  flashSale,
  loadingFlashSales,
}: IProps) {
  ...
  const onSubmit = async (values: FormValues) => {
    const cleanGroupItems = JSON.parse(
      JSON.stringify(groupItems, (key, value) =>
      ...
      const inputData = {
        ...
        settings: {
          ...
          trendingProducts: {
            ...
            banner: JSON.stringify(trendingProductsBannerData),
          },
          featuredBrands: {
          ...
          latestProducts: {
            ...
            banner: JSON.stringify(latestProductsBannerData),
          },
          // builder: JSON.stringify(builderData),
          builder: {
          ...
          action: Array.isArray(router?.query?.action)

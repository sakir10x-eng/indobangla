...
export const RELEASES: Release[] = [
  {
    ...
    items: [
      { type: 'added', text: 'E-book selling. Upload a PDF or EPUB on the product page and it becomes a readable e-book: the file is kept on the server and converted to page images, so customers read it in “আমার ই-বুক” one page at a time and can’t download or forward it — every page carries the reader’s own phone number. Set how many opening pages anyone may sample free before buying. E-book orders are bKash-only (they unlock the moment payment succeeds, so cash-on-delivery isn’t offered)' },
      { type: 'added', text: 'Order view → Notify customer: a “Send SMS” button beside WhatsApp, so the message actually reaches customers who don’t use WhatsApp (sent through the store’s SMS gateway). WhatsApp only ever opened a compose window on your own phone' },

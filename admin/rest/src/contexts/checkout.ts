...
// ---- IndoBangla manual order adjustments (admin checkout) ----
export const manualDiscountAtom = atom(
  (get) => Number(get(checkoutAtom).manual_discount ?? 0),
  (get, set, data: number) =>
    set(checkoutAtom, { ...get(checkoutAtom), manual_discount: data })
);
export const adjustmentAtom = atom(
  (get) => Number(get(checkoutAtom).adjustment ?? 0),
  (get, set, data: number) =>
    set(checkoutAtom, { ...get(checkoutAtom), adjustment: data })
);
export const advancePaidAtom = atom(
  (get) => Number(get(checkoutAtom).advance_paid ?? 0),
  (get, set, data: number) =>
    set(checkoutAtom, { ...get(checkoutAtom), advance_paid: data })
);
export const orderNoteAtom = atom(
  (get) => (get(checkoutAtom).order_note as string) ?? '',
  (get, set, data: string) =>
    set(checkoutAtom, { ...get(checkoutAtom), order_note: data })
);

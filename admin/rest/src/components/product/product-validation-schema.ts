...
// SKU is optional by default; toggle required fields from Admin → AI Settings.
export const productValidationSchema = (rules: FieldRules = {}) =>
  yup.object().shape({
  ...
  sku: yup.mixed().when('product_type', {
    ...
    then: () =>
      rules?.sku
        ? yup.string().nullable().required('form:error-sku-required')
        : yup.string().nullable(),
  }),

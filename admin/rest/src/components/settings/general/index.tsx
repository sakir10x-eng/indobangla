...
export default function GeneralSettingsForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      server_info: serverInfo,
      logo: options?.logo ?? '',
      elegantLogo: options?.elegantLogo ?? '',
  ...
  const elegantLogoInformation = (
    <span>
      {t('form:logo-help-text')} <br /> (This is for elegant layout footer
      only.)
      <br />
      {t('form:logo-dimension-help-text')} &nbsp;
      <span className="font-bold">
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="flex flex-wrap pb-8 my-5 border-b border-dashed border-border-base sm:my-8">
        <Description
          title={`Elegant ${t('form:input-label-logo')}`}
          details={elegantLogoInformation}
          className="w-full px-0 pb-5 sm:w-4/12 sm:py-8 sm:pe-4 md:w-1/3 md:pe-5"
        />

        <Card className="w-full logo-field-area sm:w-8/12 md:w-2/3">
          <FileInput name="elegantLogo" control={control} multiple={false} />
        </Card>
      </div>
